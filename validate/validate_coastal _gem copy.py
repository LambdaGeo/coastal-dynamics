...
from dissmodel.executor.schemas  import DataSource # ou de onde você importa DataSource no seu projeto
...
# ── runners (AGORA USANDO EXECUTORS) ──────────────────────────────────────────
...
def run_raster(uri: str, args) -> tuple[any, float]:
    record = ExperimentRecord(
        ...
        source        = DataSource(uri=uri, type="local"),
        input_format  = "vector", # Força a rasterização do shapefile
        parameters    = {
            "start_time":    1,
            "end_time":      args.steps,
            "taxa_elevacao": SEA_LEVEL_RISE_RATE,
